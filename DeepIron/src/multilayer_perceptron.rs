...
impl MLP {
    ...
    pub fn add_layer(&self, layer: LinearLayer, activation_function: ActivationFunctionType) -> MLP {
        let mut new_layers: Vec<LinearLayer> = self.layers.clone();
        let mut new_activation_functions: Vec<ActivationFunctionType> = self.activation_functions.clone();
    ...
    pub fn set_layer(&self, i: usize, layer: LinearLayer, activation_function: ActivationFunctionType) -> MLP {
    ...
    pub fn backward(&self, inputs: Series, outputs: Series) -> (Vec<Series>, Vec<Series>) {
        ...
        for (i, layer) in self.layers.iter().enumerate().rev() {
            let (weight_gradients, bias_gradients, input_gradients) = layer.backward(inputs.clone(), outputs.clone(), self.loss_function.clone(), self.activation_functions[i].clone(), upstream_gradient);
...
impl std::fmt::Display for MLP {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
           layers_string.push_str(&format!("Layer {}:\n\t\tNumber of neurons: {}\n", i, layer.weights.len()));
           layers_string.push_str(&format!("\t\tActivation Function: {}", self.activation_functions[i]));
        }

        write!(f, "MLP - {} layers\n
        Loss Function: {}\n
        Layers: {} \n 
        ", self.layers.len(), self.loss_function, layers_string)
    }
}